...
#2- set num plots in graphic device
par(mfrow= c(2,2))

#3- generate plot diagrams
plot(datetime, as.numeric(household_sub_data$Global_active_power), type="l", xlab="", ylab="Global Active Power (kilowatts)")

plot(datetime, as.numeric(household_sub_data$Voltage), type="l", xlab="datetime", ylab="Voltage")
...
plot(datetime, as.numeric(household_sub_data$Global_reactive_power), type="l", xlab="datetime", ylab="Global_reactive_power")
